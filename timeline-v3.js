// Timeline playback v3: fade completed route, highlight only the active leg.
(function () {
    ...
    function buildTimelineStops(source) {
        ...
        valid.forEach(loc => {
            ...
            // A one-day excursion inside a longer base stay returns to that base automatically.
    ...
    function ensureTimelineUI() {
        if (!document.getElementById('timeline-v3-style')) {
            const style = document.createElement('style');
            style.id = 'timeline-v3-style';
            style.textContent = [
        ...
        if (!hud) {
            ...
            hud.innerHTML = '<div class="timeline-hud-top"><div><div class="timeline-hud-kicker">Current leg</div><div class="timeline-hud-place"></div><div class="timeline-hud-date"></div></div><div class="timeline-hud-count"></div></div><div class="timeline-hud-progress"><span></span></div>';
    ...
    function updateHud(hud, loc, index, total, fraction = 0) {
        ...
        hud.querySelector('.timeline-hud-kicker').textContent = 'Current leg';
    ...
    function addArrivalMarker(loc, latLng) {
        const marker = L.circleMarker(latLng, {
            radius: loc.syntheticReturn ? 4 : 5,
            weight: 2,
            color: loc.syntheticReturn ? '#8a94a6' : '#69c8ff',
            fillColor: loc.syntheticReturn ? '#8a94a6' : '#69c8ff',
            fillOpacity: loc.syntheticReturn ? 0.55 : 0.72,
            opacity: loc.syntheticReturn ? 0.55 : 0.72
    ...
    async function animateLeg({ from, to, movingMarker, completedGlow, completedLine, completedPoints, activeGlow, activeLine, run, hud, stopIndex, totalStops, reducedMotion }) {
        ...
        const activePoints = [];
        const startingLng = completedPoints.length ? completedPoints[completedPoints.length - 1][1] : start[1];
        activePoints.push([start[0], startingLng]);
        activeGlow.setLatLngs(activePoints);
        activeLine.setLatLngs(activePoints);

        const finished = await new Promise(resolve => {
            const started = performance.now();
            let lastRouteSample = 0;
            let continuousLng = startingLng;
        ...
        // Once a leg is completed, move it into the subdued history layer.
        if (!completedPoints.length) completedPoints.push(...activePoints);
        else completedPoints.push(...activePoints.slice(1));
        completedGlow.setLatLngs(completedPoints);
    ...
    async function playTimelineV3() {
        ...
        const completedGlow = L.polyline([], {
            color: '#69c8ff', weight: 8, opacity: 0.035, lineCap: 'round', lineJoin: 'round'
        }).addTo(regionLayerGroup);
        const completedLine = L.polyline([], {
            color: '#93a0b3', weight: 2.5, opacity: 0.30, lineCap: 'round', lineJoin: 'round'
        }).addTo(regionLayerGroup);

        // Only the leg currently being travelled receives the bright treatment.
        const activeGlow = L.polyline([], {
            color: '#f5c842', weight: 12, opacity: 0.16, lineCap: 'round', lineJoin: 'round'
        }).addTo(regionLayerGroup);
        const activeLine = L.polyline([], {
            color: '#f5c842', weight: 4.5, opacity: 0.98, lineCap: 'round', lineJoin: 'round'
        }).addTo(regionLayerGroup);

        const movingIcon = L.divIcon({
            html: '<div class="timeline-moving-marker"></div>',
        ...
        try {
            ...
            completedPoints.push(firstPoint);
            completedGlow.setLatLngs(completedPoints);
            completedLine.setLatLngs(completedPoints);
            ...
            for (let index = 1; index < stops.length && !run.cancelled; index += 1) {
                ...
                const completed = await animateLeg({
                    from, to, movingMarker,
                    completedGlow, completedLine, completedPoints,
                    activeGlow, activeLine,
                    run, hud, stopIndex: index, totalStops: stops.length, reducedMotion
                });
                if (!completed || run.cancelled) break;

                const arrived = completedPoints[completedPoints.length - 1];
                addArrivalMarker(to, arrived);

                if (!reducedMotion) {
                    const legDistance = haversineKm(
                        [parseFloat(from.lat), parseFloat(from.lng)],
                        [parseFloat(to.lat), parseFloat(to.lng)]
                    );
                    mapMain.flyTo(arrived, Math.min(8, cameraZoomForDistance(legDistance) + 1), {
                        animate: true,
                        duration: 0.48
                    });
                }
                ...
                await wait(reducedMotion ? 35 : (to.syntheticReturn ? 260 : 480), run);
            }

            if (!run.cancelled && completedPoints.length > 1) {
                const bounds = L.latLngBounds(completedPoints);
                mapMain.fitBounds(bounds.pad(0.22), {
                ...
                await wait(reducedMotion ? 60 : 1050, run);
    ...
    // Replace the legacy listener by cloning the button, then attach v3 playback.
    ...
    if (oldButton) {
        ...
        upgradedButton.addEventListener('click', playTimelineV3);
    }
})();
