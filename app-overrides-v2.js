// Local overrides layered on top of the original full app core.
(function () {
    // ---------- Geocoding fallback ----------
    const COUNTRY_CODES = {
    ...
    function geoScore(item, region) {
        ...
        return (target && name.includes(target) ? 4 : 0)
            + (polygon ? 3 : 0)
    ...
    async function directGeocode(region, country = '') {
        ...
        for (const variant of variants) {
            ...
            try {
                ...
                console.warn('Direct Nominatim fallback failed:', e);
            } finally {
                clearTimeout(timer);
            }
        }
    ...
    fetchRegionBoundary = async function(region, country = '') {
        ...
        if (backendBoundary) {
            try {
                ...
                console.warn('Backend geocode failed; using fallback:', e);
            }
        }
        return normalizeGeo(await directGeocode(r,c), c, r);
    };

    // ---------- Country map: coordinates first ----------
    ...
    function countryFeatureIndex(lat, lng) {
        ...
        for (let i=0; i<worldGeoJSON.features.length; i++) {
            try {
                if (worldGeoJSON.features[i]?.geometry && turf.booleanPointInPolygon(point, worldGeoJSON.features[i])) return i;
            } catch (_) {}
        }
        return null;
    }
    ...
    renderMapCountries = function(sourceLocations = locations) {
        ...
        sourceLocations.forEach(loc => {
            ...
            if (idx !== null) byIndex.set(idx, (byIndex.get(idx) || 0) + days);
                ...
                const key = cleanCountry(loc.country);
                if (key) byName.set(key, (byName.get(key) || 0) + days);
            }
        });
        ...
        L.geoJSON(worldGeoJSON, {
            style: feature => {
                ...
                if (!days) {
                    ...
                    for (const raw of [p.name,p.ADMIN,p.NAME,p.NAME_EN].filter(Boolean)) {
                        days += byName.get(cleanCountry(raw)) || 0;
                    }
                }
                ...
                return {
                    ...
                    fillOpacity: visited ? 0.4 + 0.5 * (days / maxDays) : 0.2
                };
            }
        }).addTo(countryLayerGroup);

        const pct = document.getElementById('explore-percent');
    ...
    function installRankingUI() {
        ...
        if (insights && !document.getElementById('city-ranking-list')) {
    ...
    function cityKey(loc) {
        ...
        const norm = v => String(v || '').trim().toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9\u3400-\u9fff]+/g,' ').trim();
    ...
    function cityRankingData(source = locations) {
        ...
        return [...map.values()]
            .map(s => ({...s, rank:Number.isFinite(parseInt(s.latest?.ranking,10)) ? parseInt(s.latest.ranking,10) : s.best}))
            .filter(s => s.latest)
            .sort((a,b) => (a.rank || 9999) - (b.rank || 9999) || b.totalDays - a.totalDays);
    }

    function renderCityRanking() {
        const root = document.getElementById('city-ranking-list');
        if (!root) return;
        const cities = cityRankingData(locations);
        if (!cities.length) {
            root.innerHTML = '<div class="city-ranking-empty">目前沒有城市排名。</div>';
            return;
        }
        root.innerHTML = cities.map(s => {
            const l = s.latest;
            const rank = Number.isFinite(s.rank) ? '#' + s.rank : '—';
    ...
    const coreRenderAll = typeof renderAll === 'function' ? renderAll : null;
    if (coreRenderAll) {
        renderAll = function(...args) {
            const result = coreRenderAll.apply(this, args);
            renderCityRanking();
            return result;
        };
    }

    // Remove ranking badge from timeline rows; ranking now lives in Insights.
    ...
    function syncEndDateMinimum({repairValue = false} = {}) {
        ...
        if (repairValue && start && endInput.value && endInput.value < start) endInput.value = start;
    }
